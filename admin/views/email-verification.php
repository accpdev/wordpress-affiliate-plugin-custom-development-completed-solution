<form  method="post">
	<div class="uap-stuffbox">
		<h3 class="uap-h3"><?php esc_html_e('E-mail Verification', 'uap');?><span class="uap-admin-need-help"><i class="fa-uap fa-help-uap"></i><a href="https://help.wpindeed.com/ultimate-affiliate-pro/knowledge-base/e-mail-verification/" target="_blank"><?php esc_html_e('Need Help?', 'uap');?></a></span></h3>
		<div class="inside">
						...
						<span class="uap-labels-special"><?php esc_html_e('Activation Link Expire Time:', 'uap');?></span>
							<select name="uap_double_email_expire_time">
								<?php
									...
									foreach ($arr as $k=>$v){
										?>
										<option value="<?php echo $k?>" <?php echo ($k==$data['metas']['uap_double_email_expire_time']) ? 'selected' : '';?> >
											<?php echo $v;?>
										</option>
										<?php
									}
								...
								<?php
									if ($data['pages']){
										foreach ($data['pages'] as $k=>$v){
											?>
												<option value="<?php echo $k;?>" <?php echo ($data['metas']['uap_double_email_redirect_success']==$k) ? 'selected' : '';?> ><?php echo $v;?></option>
											<?php
										}
									}
								?>
							...
							<select name="uap_double_email_redirect_error">
								...
								<?php
									if ($data['pages']){
										foreach ($data['pages'] as $k=>$v){
											?>
												<option value="<?php echo $k;?>" <?php echo ($data['metas']['uap_double_email_redirect_error']==$k) ? 'selected' : '';?> ><?php echo $v;?></option>
											<?php
										}
									}
								?>
						...
						<span class="uap-labels-special"><?php esc_html_e('Delete User if is not verified:', 'uap');?></span>
							<select name="uap_double_email_delete_user_not_verified">
								<?php
									...
									foreach ($arr as $k=>$v){
										?>
										<option value="<?php echo $k?>" <?php echo ($k==$data['metas']['uap_double_email_delete_user_not_verified']) ? 'selected' : '';?> >
											<?php echo $v;?>
										</option>
										<?php
									}
